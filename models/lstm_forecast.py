...
def lstm(T):
    ...
    model.compile(
            loss='mse',
            optimizer=Adam(learning_rate=0.1)  # Updated from lr to learning_rate
    )
...
def plot_loss(model, ticker):
    # Disable plotting to prevent system from getting stuck
    print(f"\n{ticker} Model Training Results:")
    print(f"Final Training Loss: {model.history['loss'][-1]:.4f}")
...
def plot_predictions(predictions, Y, ticker):
    # Disable plotting to prevent system from getting stuck
    print(f"\n{ticker} Model Forecast Results:")
    print(f"Targets: {Y[:5]}... (showing first 5 values)")
    print(f"Predictions: {predictions[:5]}... (showing first 5 values)")
...
def onestep_forecast(model, T, X, Y, N, ticker):
    ...
    # Return as 2D array for scaler.inverse_transform
...
def multistep_forecast(model, T, X, Y, N, ticker):
    ...
    # Return as 2D array for scaler.inverse_transform
    return np.array(validation_predictions).reshape(-1, 1)
...
class LSTMForecaster:
    def __init__(self, forecast_window=10):
    ...
    def fit(self, series):
        ...
        self.scaler.fit(series[:len(series) // 2])
        series_scaled = self.scaler.transform(series).flatten()
        T = self.forecast_window
        X, Y = [], []
        ...
        Y = np.array(Y)
        self.model = lstm(T)
        self.model.fit(X[:-len(X)//2], Y[:-len(Y)//2], epochs=100, validation_data=(X[-len(X)//2:], Y[-len(Y)//2:]))

    def predict(self, series, mode="one"):
        T = self.forecast_window
        X, Y = [], []
        series_scaled = self.scaler.transform(series).flatten()
        for t in range(len(series_scaled) - T):
        ...
        if mode == "one":
            ...
            # Multi-step forecast logic (reuse existing)
            validation_target = Y[-len(Y)//2:]
            validation_predictions = []
            last_x = X[-len(X)//2]
            while len(validation_predictions) < len(validation_target):
                p = self.model.predict(last_x.reshape(1, T, 1))[0,0]
                validation_predictions.append(p)
                last_x = np.roll(last_x, -1)
                last_x[-1] = p
            return self.scaler.inverse_transform(np.array(validation_predictions).reshape(-1,1)).flatten()
