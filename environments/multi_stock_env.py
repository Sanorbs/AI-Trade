...
class MultiStockEnv(gym.Env):
    ...
    def __init__(self, data, n_stock, n_forecast, n_sentiment, initial_investment, agent):
        ...
        if self.agent == "DQN":
            ...
            self.action_space = spaces.Box(low = np.array([0.0]*self.n_stock), 
                                           high = np.array([2.0]*self.n_stock),
                                           dtype=np.float32)
    ...
    def _trade(self, action):
        ...
        if self.agent == "DQN":
            ...
            # For DDPG, action is already a continuous array
            action_vec = action
        ...
        for i, a in enumerate(action_vec):
            # For continuous actions, use thresholds
            if a < 0.5:  # sell threshold
                sell_index.append(i)
            elif a > 1.5:  # buy threshold
                buy_index.append(i)
            # else: hold (0.5 <= a <= 1.5)
